...
const vertexShader = `
  ...
  void main() {
    ...
    // Add noise to instance position based on time
    ...
    // Billboarding: scale based on instanceScale and keep facing camera
...
// A simple PRNG to avoid Math.random() impurity warnings in React 19
...
export default function GPUFluidParticles({ count = 10000, color = '#00ffcc' }) {
  ...
  const [positions, scales] = useMemo(() => {
    ...
    const random = mulberry32(1337) // Seeded random for deterministic behavior
    for (let i = 0; i < count; i++) {
      // Scatter particles in a sphere
      const theta = random() * Math.PI * 2
      const phi = Math.acos((random() * 2) - 1)
      const r = Math.cbrt(random()) * 5 // Radius of 5
